...
public class WebClientImpl implements WebClient {
            ...
            if (asyncResult.succeeded()) {
                ...
                webSocket.handler(buffer -> {
                    ...
                    if (jsonMessage.containsKey("event") &&
                            jsonMessage.getString("event").equals(BIKE_CHANGE_EVENT_TOPIC) &&
    ...
    public void startMonitoringUsers(EBikeApp app) {
        vertx.createHttpClient().webSocket(SERVER_PORT, SERVER_HOST, "/api/users/monitoring", asyncResult -> {
            if (asyncResult.succeeded()) {
                WebSocket webSocket = asyncResult.result();
                webSocket.handler(buffer -> {
                    String message = buffer.toString();
                    JsonObject jsonMessage = new JsonObject(message);
                    if (jsonMessage.containsKey("event") &&
                            jsonMessage.getString("event").equals(USER_CHANGE_EVENT_TOPIC) &&
                            jsonMessage.containsKey(USER_ID) && jsonMessage.containsKey(CREDIT)){
